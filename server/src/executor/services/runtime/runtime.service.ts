...
export class RuntimeService {
  ...
  async runWithBreakpoints(code: string): Promise<void> {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      // Регистрируем listener для паузы ТОЛЬКО для sync кода
      ...
      try {
        ...
        console.log('✅ Code execution started (Hybrid mode: sync steps + Event Loop tracking)');

        // 🎯 Start tracking user code RIGHT BEFORE running user code
        // This ensures we skip V8 Inspector/breakpoint setup promises
        this.eventLoopTracker.startTrackingUserCode();

        // Запускаем код
        // StepCollector автоматически пропустит async callbacks (resume вместо stepOver)
        script.runInContext(context, {
          timeout: this.TIMEOUT,
        });

        // Ждём завершения обработки sync steps
        setImmediate(async () => {
          try {
            console.log(
            ...
            // Регистрируем async операции из sync кода
            this.registerAsyncOperationsFromSteps(codeLines);

            // Останавливаем Event Loop Tracker СРАЗУ
            this.eventLoopTracker.disable();

            clearTimeout(timeout);
            console.log('✅ Code execution completed (Hybrid mode - sync only)');
            resolve();
          } catch (error) {
            this.eventLoopTracker.disable();
            clearTimeout(timeout);
  ...
  /**
   * Регистрирует async операции из sync кода
   */
  private registerAsyncOperationsFromSteps(codeLines: string[]): void {
    for (let i = 0; i < codeLines.length; i++) {
      ...
      if (asyncCheck.isAsync && asyncCheck.type) {
        this.asyncStepCollector.registerAsyncOperation(i, asyncCheck.type);
        console.log(
